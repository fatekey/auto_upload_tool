...
class httpcon(object):
    ...
    def _get_protocol(self):
        if not self.protocol:
            ps = ('PROTOCOL_SSLv23', 'PROTOCOL_TLSv1', 'PROTOCOL_SSLv2',
    ...
    def _make_con(self, scheme, host, port, proxy=None):
        if "https" != scheme:
            if proxy:
                con = client.HTTPConnection(proxy[0],
                                            int(proxy[1]),
        ...
        for p in self.protocol:
            ...
            try:
                if proxy:
                    con = client.HTTPSConnection(proxy[0],
                                                 proxy[1],
                                                 context=context,
                                                 timeout=self.timeout)
                    con.set_tunnel(host, port)
...
class hackRequests(object):
    ...
    def httpraw(self, raw: str, **kwargs):
        ...
        log["response"] = "HTTP/%.1f %d %s" % (
            rep.version * 0.1, rep.status, rep.reason) + '\r\n' + str(rep.msg)
        if port == 80 or port == 443:
            _url = "{scheme}://{host}{path}".format(scheme=scheme,
                                                    host=host,
                                                    path=path)
        else:
            _url = "{scheme}://{host}{path}".format(scheme=scheme,
    ...
    def http(self, url, **kwargs):
        ...
        if 'Accept-Encoding' not in headers:
            tmp_headers['Accept-Encoding'] = tmp_headers.get(
                "Accept-Encoding", "gzip, deflate")
        if 'Connection' not in headers:
            tmp_headers['Connection'] = 'close'
        if 'User-Agent' not in headers:
            tmp_headers[
                'User-Agent'] = tmp_headers['User-Agent'] if tmp_headers.get(
                    'User-Agent'
